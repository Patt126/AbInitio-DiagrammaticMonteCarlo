...
def read_epmatwp(bin_file, nbnd, nrr_k, nmodes, nrr_g,R,PLOT=False,outdir=""):
    ...
    # Read Raw Bytes (Complex128 = 16 bytes per number)
    G_raw = np.fromfile(bin_file, dtype=np.complex128)

    # Reshape: Fortran Order: (Band, Band, Re, Mode, Rp)
    G_ftn = G_raw.reshape((nbnd, nbnd, nrr_k, nmodes, nrr_g), order='F')

    # Transpose to Python Logic ->New Order: (Re, Rp, Mode, Band, Band)
    ...
    if(PLOT):
        ...
        ax[0].set_title(r"Electronic Position Decay ($R_p=0$)")
        ax[0].set_xlabel(r"Distance $|\mathbf{R}_e|$ ($\AA$)")
        ax[0].set_ylabel("Magnitude (Ry)")
        ax[0].grid(True, alpha=0.3)
        ...
        ax[1].set_title(r"Supercell Index Decay ($R_e=0$)")
        ax[1].set_xlabel(r"Distance $|\mathbf{R}_p|$ ($\AA$)")
        ax[1].grid(True, alpha=0.3)
...
def read_epb_mixed_record(filename, nkc, nbnd, nmodes):
    ...
    # Read the full record as raw bytes
    ...
    # Transpose to Python shape: (nqc, nkc, nmodes, nbnd, nbnd)
    # (4, 3, 2, 0, 1)
