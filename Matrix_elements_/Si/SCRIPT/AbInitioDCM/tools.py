    ...
    for n1 in range_1:
        for n2 in range_2:
            for n3 in range_3:
                ...
                #Calculate Distance of current point R
                R = n1*at[:,0] + n2*at[:,1] + n3*at[:,2]
                dist_R = np.linalg.norm(R)
                #  Wigner-Seitz Check (Is it the closest image?)
                is_in_ws = True
                ...
                # If valsorted_id, APPEND to list (NO SORTING)
                if is_in_ws:
...
def plot_EPC(g_bloch_full,g_bloch_perc,w_full,w_perc,nbnd,nmodes,x_axis,x_ticks,x_ticks_labels,perc,outdir = ""):
    ...
    # according to eq. (9): D = (1/ħ) * sqrt( 2 ω M_uc * (1/Nb) * sum |g|^2 ) in Lou
...
def get_random_k_primitive(b_lat, n_points=1000):
    ...
    """
    #Generate random fractional coordinates [-0.5, 0.5)
...
# Performance Benchmark of Wannier interpolation
...
def performance_analysis_vectorized(N,b_lat,H_sort,D_sort,G,R_sort,U,Vt,s,outdir=""):
    ...
    print(K.shape)
    start_time = time.time()
    g,_,_ = BENCHMARK(H_sort,D_sort,G,R_sort,K,np.array([0,0,0]))
    time_counter[-1]+= time.time()-start_time
    print("full done")
    norm += np.sum(np.abs(g)**2)
    for i_r in range(points-1):
        ...
        s_red = s[:ranks[i_r]]
        start_time = time.time()
        g_r,_,_ = Wannier2Bloch(H_sort,D_sort,G,U_red,Vt_red,s_red,R_sort,K,np.array([0,0,0]))
        time_counter[i_r]+= time.time()-start_time 
        print(f"rank {ranks[i_r]} done,shape:",g_r.shape)
        errors[i_r] += np.sum(np.abs(g_r - g)**2)
